...
const DB = {
  ...
  async set(type, data) {
    ...
    State._dataVersion = (State._dataVersion || 1) + 1;
    State._tabStamp = {};
    Engine.checkAlerts();
  },
...
const UI = {
  ...
  resetWheel() { State.wheelList = [...PupilMgr.getPupilNames()]; State.wA = 0; setTxt('wheelWinner', ''); this.drawWheel(0); },
  ...
  startTimer(sec) {
    ...
    const paint = () => {
      const m = Math.floor(Math.max(t,0) / 60), s = Math.max(t,0) % 60;
      setTxt('timerDisplay', `${m.toString().padStart(2,'0')}:${s.toString().padStart(2,'0')}`);
      el.className = t <= 10 && t > 0 ? 'urgent' : '';
    };
    paint();
...
<!-- QUICK ACTIONS -->
<button id="quickFab" class="no-print" onclick="UI.toggleQuickActions()">＋</button>
<div id="quickActionsMenu" class="no-print">
  <button class="quick-action-btn" onclick="UI.quickGo('attendance')">📅 تسجيل غياب</button>
  <button class="quick-action-btn" onclick="UI.quickGo('notes')">📋 إضافة ملاحظة</button>
  <button class="quick-action-btn" onclick="UI.quickGo('grades')">💯 إدخال نقطة</button>
  <button class="quick-action-btn" onclick="UI.quickGo('homework')">📝 واجب منزلي</button>
</div>
...
const ParentReport = {
  ...
  print() {
    ...
    const w = window.open('','_blank');
    w.document.write(`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>تقرير المتعلم</title><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@700;900&family=Tajawal:wght@400;700&display=swap" rel="stylesheet"></head><body>${content.outerHTML}
...
const Journal = {
  ...
  async exportPrintablePDF() {
    ...
    const blob = new Blob([`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>المذكرة_${data.currentWeekId}</title></head><body>${daysHtml}
...
'today-class':'🎯 حصة اليوم',
};
async function switchTab(tabId) {
  const aliasTabs = { 'support-groups':'tools', 'journal':'sessions', 'hypothesis':'dashboard', 'equity':'dashboard', 'biography':'reports' };
  tabId = aliasTabs[tabId] || tabId;
  document.querySelectorAll('.tab-content').forEach(t => t.classList.remove('active'));
  document.querySelectorAll('.nav-item').forEach(b => b.classList.remove('active'));
  document.querySelectorAll('.mobile-bottom-nav .mb-item').forEach(b => b.classList.remove('active'));
  const tab = document.getElementById('tab-' + tabId);
  const btn = document.getElementById('btn-' + tabId);
  const mobileBtn = document.getElementById('mb-' + tabId);
  if (tab) tab.classList.add('active');
  if (btn) btn.classList.add('active');
  ...
  if (tabId !== 'tools') document.body.classList.remove('wheel-mobile-focus');
...
function toggleWheelFocus(force) {
  if (window.innerWidth > 768) return;
  const shouldOpen = typeof force === 'boolean' ? force : !document.body.classList.contains('wheel-mobile-focus');
  if (shouldOpen && State.tab !== 'tools') switchTab('tools');
  document.body.classList.toggle('wheel-mobile-focus', shouldOpen);
  if (shouldOpen) setTimeout(() => UI.drawWheel(State.wA || 0), 60);
}
...
function enhanceTablesForMobile() {
  ...
  document.querySelectorAll('.tab-content.active table').forEach(table => {
    ...
    table.closest('.card, .table-wrap, .table-responsive, .tab-content, .modal-body')?.classList.add('mobile-stack-table');
    table.querySelectorAll('tbody tr').forEach(tr => {
      [...tr.children].forEach((td, i) => td.setAttribute('data-label', headers[i] || 'حقل'));
    });
...
function updateMobileShell() {
  ...
  const fab = document.getElementById('mobileFabMenu');
  if (fab) fab.style.display = isMobile && !document.body.classList.contains('wheel-mobile-focus') ? 'flex' : 'none';
  const strip = document.getElementById('mobileHomeStrip');
  if (strip) strip.style.display = isMobile ? 'flex' : 'none';
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    document.body.classList.remove('wheel-mobile-focus');
    updateMobileShell();
  }
});

async function refreshCurrentTab(tabId, force = false) {
  const t = tabId || State.tab;
  if (State._refreshing === t) return;
  const stamp = `${State.year}|${State.level}|${State._dataVersion}`;
  if (!force && State._tabStamp && State._tabStamp[t] === stamp && t !== 'tools') return;
  State._refreshing = t;
  try {
    switch (t) {
      ...
      case 'today-class': await UI.renderTodayClass(); break;
      case 'tools':
        if (!Array.isArray(State.wheelList) || !State.wheelList.length) UI.resetWheel();
        else UI.drawWheel(State.wA || 0);
        break;
    }
    State._tabStamp[t] = stamp;
  } finally {
...
function changeYear(y) { if (State.year === y) return; State.year = y; State._dataVersion++; State._tabStamp = {}; Engine.invalidate(); initDropdowns(); refreshCurrentTab(State.tab, true); }
function changeLevel(l) { if (State.level === l) return; State.level = l; State._dataVersion++; State._tabStamp = {}; Engine.invalidate(); initDropdowns(); refreshCurrentTab(State.tab, true); }
...
function printCurrentTab() {
  ...
  const title = (document.getElementById('tabTitle')?.textContent || 'طباعة').trim();
  ...
  header.innerHTML = `
    <h1 style="font:900 22px Cairo,sans-serif;margin:0 0 8px">${title}</h1>
    <div style="font-size:12px;color:#444">التاريخ: ${today} | المستوى: ${level} | السنة: ${year}</div>
  `;
  activeTab.prepend(header);
...
UI.toggleQuickActions = function(){
  const menu = document.getElementById('quickActionsMenu');
  if (!menu) return;
  State.quickActionsOpen = !State.quickActionsOpen;
  menu.classList.toggle('open', State.quickActionsOpen);
};

UI.quickGo = async function(tabId){
...
async function printWeeklySummary() {
  ...
  const gradeRows = pupils.map(name => {
    const g = grades[name] || [];
    const nums = g.map(v => +v).filter(v => !Number.isNaN(v));
    const avg = nums.length ? (nums.reduce((a,b)=>a+b,0) / nums.length).toFixed(1) : '—';
    return `<tr><td>${name}</td><td>${avg}</td></tr>`;
  }).join('');

  const attRows = pupils.map(name => {
    let absent = 0, late = 0;
    ...
    return `<tr><td>${name}</td><td>${absent}</td><td>${late}</td></tr>`;
  }).join('');

  const hwRows = pupils.map(name => {
    let done = 0, missing = 0;
    Object.values(homework || {}).forEach(day => {
      const st = day?.[name];
      if (st === 'done') done++;
      if (st === 'missing') missing++;
    });
    return `<tr><td>${name}</td><td>${done}</td><td>${missing}</td></tr>`;
  }).join('');

  const noteRows = (notes || []).slice(0, 12).map(n =>
    `<tr><td>${n.student || '—'}</td><td>${n.type || '—'}</td><td>${n.text || '—'}</td><td>${n.date || '—'}</td></tr>`
  ).join('') || '<tr><td colspan="4">لا توجد ملاحظات</td></tr>';

  const html = `
    <html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>تقرير أسبوعي</title>
    </head><body>
      ${buildPrintHeader('التقرير الأسبوعي للقسم', teacher, school, year, level)}
      <div class="block"><h2>الفروض</h2><table><thead><tr><th>الاسم</th><th>المعدل</th></tr></thead><tbody>${gradeRows}</tbody></table></div>
      <div class="block"><h2>الغياب والتأخر</h2><table><thead><tr><th>الاسم</th><th>عدد الغياب</th><th>عدد التأخر</th></tr></thead><tbody>${attRows}</tbody></table></div>
      <div class="block"><h2>الواجبات المنزلية</h2><table><thead><tr><th>الاسم</th><th>منجز</th><th>غير منجز</th></tr></thead><tbody>${hwRows}</tbody></table></div>
      <div class="block"><h2>آخر الملاحظات</h2><table><thead><tr><th>الاسم</th><th>النوع</th><th>الملاحظة</th><th>التاريخ</th></tr></thead><tbody>${noteRows}</tbody></table></div>
    </body></html>
  `;
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && document.body.classList.contains('wheel-mobile-focus')) toggleWheelFocus(false);
});
window.addEventListener('resize', () => {
  if (window.innerWidth > 768) document.body.classList.remove('wheel-mobile-focus');
});

updateMobileShell();
setTimeout(updateMobileShell, 300);



(function(){
  const SUPABASE_URL = 'https://doxaczzvjucxxjovgivv.supabase.co';
  const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRveGFjenp2anVjeHhqb3ZnaXZ2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM0MzUwMzUsImV4cCI6MjA4OTAxMTAzNX0.ODwr8kEbMu3FS_8V8Hxlh-jh5Vn9PRggMXQfRhxULno';
  const $ = (id)=>document.getElementById(id);
  let client = null;
  function status(msg, err){ const el=$('sbCloudStatus'); if(el){ el.textContent=msg; el.style.color = err ? '#b91c1c' : '#334155'; } console[err?'error':'log']('[cloud]', msg); }
  function setBusy(ids, busy){ ids.forEach(id=>{ const el=$(id); if(el) el.disabled=busy; }); }
  function panel(open){ const el=$('sbCloudPanel'); if(!el) return; el.classList.toggle('open', open); el.setAttribute('aria-hidden', open ? 'false' : 'true'); }
  function initClient(){
    try {
      if (!window.supabase || !window.supabase.createClient) throw new Error('لم يتم تحميل مكتبة Supabase');
      client = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
      return client;
    } catch (e) { status(e.message || 'تعذر تهيئة Supabase', true); return null; }
  }
  ...
  function importLocal(payload){
    if (!payload || typeof payload !== 'object') throw new Error('لا توجد بيانات صالحة');
    Object.keys(payload).forEach(k=>{
      if (k && !k.startsWith('sb:')) localStorage.setItem(k, payload[k]);
    });
  }
  async function getUser(){
    if (!client) initClient();
    if (!client) throw new Error('Supabase غير جاهز');
  ...
  async function signIn(){
    try {
      setBusy(['sbSignIn','sbSignUp'], true); status('جارٍ تسجيل الدخول...');
      initClient();
      const email = $('sbEmail').value.trim(); const password = $('sbPassword').value;
      const { error } = await client.auth.signInWithPassword({ email, password });
      if (error) throw error;
      status('تم تسجيل الدخول بنجاح.');
    } catch(e){ status(e.message || 'فشل تسجيل الدخول', true); }
    finally{ setBusy(['sbSignIn','sbSignUp'], false); }
  }
  async function signUp(){
    try {
      setBusy(['sbSignIn','sbSignUp'], true); status('جارٍ إنشاء الحساب...');
      initClient();
      const email = $('sbEmail').value.trim(); const password = $('sbPassword').value;
      const { error } = await client.auth.signUp({ email, password });
      if (error) throw error;
      status('تم إنشاء الحساب. إن كان التأكيد مفعلاً فافتح بريدك أولاً.');
    } catch(e){ status(e.message || 'فشل إنشاء الحساب', true); }
    finally{ setBusy(['sbSignIn','sbSignUp'], false); }
  }
  async function pushCloud(){
    try {
      setBusy(['sbPush','sbPull','sbTest'], true); status('جارٍ رفع البيانات...');
      initClient();
      const user = await getUser();
      if (!user) throw new Error('سجل الدخول أولاً');
      const payload = exportLocal();
      ...
      if (error) throw error;
      status('تم رفع البيانات إلى السحابة.');
    } catch(e){ status(e.message || 'فشل الرفع', true); }
    finally{ setBusy(['sbPush','sbPull','sbTest'], false); }
  }
  async function pullCloud(){
    try {
      setBusy(['sbPush','sbPull','sbTest'], true); status('جارٍ استرجاع البيانات...');
      initClient();
      const user = await getUser();
      if (!user) throw new Error('سجل الدخول أولاً');
      const { data, error } = await client.from('app_snapshots').select('payload,updated_at').eq('user_id', user.id).single();
      if (error) throw error;
      importLocal(data.payload || {});
      status('تم الاسترجاع. سيعاد تحميل التطبيق الآن.');
      setTimeout(()=>location.reload(), 700);
    } catch(e){ status(e.message || 'فشل الاسترجاع', true); }
    finally{ setBusy(['sbPush','sbPull','sbTest'], false); }
  }
  async function testConnection(){
    try {
      setBusy(['sbPush','sbPull','sbTest'], true); status('جارٍ اختبار الاتصال...');
      initClient();
      const { error } = await client.from('app_snapshots').select('user_id').limit(1);
      if (error && !String(error.message||'').includes('JWT')) throw error;
      status('الاتصال مع Supabase يعمل.');
    } catch(e){ status(e.message || 'تعذر الاتصال', true); }
    finally{ setBusy(['sbPush','sbPull','sbTest'], false); }
  }
  async function signOut(){
    try { initClient(); const { error } = await client.auth.signOut(); if (error) throw error; status('تم تسجيل الخروج.'); }
    catch(e){ status(e.message || 'تعذر تسجيل الخروج', true); }
  }
  document.addEventListener('DOMContentLoaded', function(){
    $('sbCloudFab')?.addEventListener('click', ()=>panel(!$('sbCloudPanel').classList.contains('open')));
    $('sbClose')?.addEventListener('click', ()=>panel(false));
    $('sbSignIn')?.addEventListener('click', signIn);
    $('sbSignUp')?.addEventListener('click', signUp);
    $('sbPush')?.addEventListener('click', pushCloud);
    $('sbPull')?.addEventListener('click', pullCloud);
    $('sbTest')?.addEventListener('click', testConnection);
    $('sbSignOut')?.addEventListener('click', signOut);
    initClient();
  });
})();
